...
import { styles } from './RentInvoiceScreen.styles';
import { invoiceService } from '@/shared/api/services/invoiceService';
import { ConfirmModal } from '@/shared/components';

export const RentInvoiceScreen = () => {
  ...
  // Block 1: Amount Info
  const [amountStr, setAmountStr] = useState('3,000,000');
  ...
  useEffect(() => {
    const fetchInvoiceData = async () => {
      ...
      try {
        ...
        const data = res.data;
        if (data) {
          setAmountStr(data.amount?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") || '3,000,000');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const invoiceName = `Tiền nhà tháng ${monthLabel}`;
    ...
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        <LinearGradient colors={['#FFD6EC', '#E9D5FF', '#BFDBFE']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.header}>
          ...
          <View style={styles.headerTop}>
            ...
            <View style={styles.titleContainer}>
              <Text style={styles.headerTitle}>{editId ? "Sửa hóa đơn" : "Tiền thuê nhà"}</Text>
              <Text style={styles.headerSubtitle}>Quản lý hợp đồng & cước phòng</Text>
            </View>
          </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <View style={styles.card}>
          <Text style={styles.cardTitle}>Thông tin tiền phòng</Text>
